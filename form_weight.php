...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Calculate total weight if both quantity and weight are provided
$totalWeight = null;
...
if ($quantity !== null) {
   ...
   if ($size !== null) {
    ...
    // Insert data into `weight_tbl` table (including total weight)
    if ($weight !== null) {
        $stmtWeight = $conn->prepare("INSERT INTO weight_tbl (weight, total_weight, product_id) VALUES (?, ?, ?)");
        $stmtWeight->bind_param("ddi", $weight, $totalWeight, $productId);
        $stmtWeight->execute();
    }
         // Insert data into `weight_tbl` table (including total weight)
    if ($weight !== null) {
        $stmtWeightCost = $conn->prepare("INSERT INTO weight_cost (weight, total_weight, product_id) VALUES (?, ?, ?)");
        $stmtWeightCost->bind_param("ddi", $weight, $totalWeight, $productId);
        $stmtWeightCost->execute();
    }
    ...
    function navigateToForm() {
        ...
        if (selectedValue) {
            ...
            <div class="form-group">
                ...
                <input type="text" id="quantity" name='size' placeholder="Enter quantity">
            </div>
            <div class="form-group">
